include(ARGS[1] * "reduce_labels_ip.jl")
using CSV
using DataFrames
...
function get_data(args)
    ...
    r_path = args[2] * "fruits_sim/class_sim.csv"
    t_path = args[2] * "fruits_sim/comb_sim.csv"
...
function reshape_data(r, t, C::Int, k::Int)
...
for k = 2:(C-1)
    ...
    tmp_r, tmp_t = reshape_data(r, t, C, k)

    # Run the optimization problem
    tmp_path = ARGS[2] * "solver_output/ip_output_k" * string(k) * ".txt"
    tmp_map = label_reduce(C, k, tmp_t, tmp_r, tmp_path)
    tmp_map = convert(DataFrame, tmp_map)

    # Save the map to disk
    CSV.write(ARGS[2] * "fruit_maps/label_map_" * string(k) * ".csv",
    tmp_map, header=false)

    # Display our progress
    if k % 5 == 0
        println("Completed $k problems")
    end
end
