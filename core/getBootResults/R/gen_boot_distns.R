...
gen_boot_distn = function(wd) {
  ...
  # We accidentally have two things named metric and so we need to correct
  # this to avoid naming confusion
  exp_df = dplyr::rename(exp_df, sim_metric = .data$metric)
  ...
  # Count the number of unique estimators in the data
  n_estimators = length(unique(df$estimator))

  # Save the plot to disk
  base_folder = basename(wd)
  filename = paste(base_folder, 'boot_distn.pdf', sep='_')
